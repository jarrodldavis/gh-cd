...
func assertParse(input string, want parsed, wantErr string) func(t *testing.T) {
	return func(t *testing.T) {
		t.Helper()
		got, err := parse(input)

		if (err == nil) != (wantErr == "") {
			t.Errorf("parse(%#v) = %s, %v, want %s, %v", input, got, err, want, wantErr)
		} else if diff := cmp.Diff(want, got, diffOpts); diff != "" {
			t.Errorf("parse(%#v) = %s, %v, want %s, %v:\n%s", input, got, err, want, wantErr, diff)
		}
	}
}
